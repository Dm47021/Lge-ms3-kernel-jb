...
void set_usb_lock(int lock)
{
	...
	if (lock){
		user_diag_enable = 0;
		lg_set_usb_lock_state(0);
	}
	else{
		user_diag_enable = 1;
		lg_set_usb_lock_state(1);
	}
}
...
#if defined(CONFIG_LGE_USB_GADGET_NDIS_VZW_DRIVER) || defined(CONFIG_LGE_USB_GADGET_NDIS_UNITED_DRIVER_NET_MODE)
...
#ifdef CONFIG_LGE_USB_GADGET_LLDM_DRIVER
static char *usb_functions_lldm[] = {
	"acm",
	"diag",
	"nmea",
	"cdc_ethernet",
...
static struct android_usb_product usb_products[] = {
...
#endif
	{	
		.product_id = 0x632F, //2012.01.31 VM696 UMS only PID
	...
	{
		.product_id = 0x6310,
		.num_functions	= ARRAY_SIZE(usb_functions_lldm),
		.functions	= usb_functions_lldm,
#ifdef CONFIG_LGE_USB_GADGET_FUNC_BIND_ONLY_INIT
		.unique_function = LLDM,
#endif	
	},
#endif
#if 1//def LG_FW_SPG_PORT
...
#endif
#if 1//def LG_FW_SPG_PORT
struct android_usb_platform_data android_usb_pdata_spg_port = {
	.vendor_id	= 0x1004,
	.product_id	= 0x632F,
	.version	= 0x0100,
...
#ifdef CONFIG_LGE_USB_GADGET_LLDM_DRIVER
struct android_usb_platform_data android_usb_pdata_lldm = {
	.vendor_id	= 0x1004,
	.product_id	= 0x6310,
	.version	= 0x0100,
	.product_name		= "LG Android USB Device",
...
static struct platform_device android_usb_device = {
	...
	.dev		= {
#ifdef CONFIG_MACH_LGE_M3S	
#if defined (CONFIG_LGE_USB_GADGET_NDIS_DRIVER) || defined(CONFIG_LGE_USB_GADGET_NDIS_VZW_DRIVER) || defined(CONFIG_LGE_USB_GADGET_NDIS_UNITED_DRIVER_NET_MODE)
...
static struct msm_gpio msm_i2c_gpios_hw[] = {
	{ GPIO_CFG(77, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA), "i2c_scl" },
	{ GPIO_CFG(78, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA), "i2c_sda" },
};

static struct msm_gpio msm_i2c_gpios_io[] = {
	{ GPIO_CFG(77, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA), "i2c_scl" },
	{ GPIO_CFG(78, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA), "i2c_sda" },
};
...
static struct msm_gpio sdc1_cfg_data[] = {
	{GPIO_CFG(58, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA), "sdc1_clk"},
	{GPIO_CFG(59, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA), "sdc1_cmd"},
	{GPIO_CFG(60, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA), "sdc1_dat_3"},
	{GPIO_CFG(61, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA), "sdc1_dat_2"},
	{GPIO_CFG(62, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA), "sdc1_dat_1"},
	{GPIO_CFG(63, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA), "sdc1_dat_0"},
};
...
static uint32_t msm_sdcc_setup_vreg(int dev_id, unsigned int enable)
{
  ...
  #if 1 // LS696_me : CAYMAN merge : Power cycle reinit sd
  
//	printk(KERN_ERR "%s: dev_id = %d enable = %d enable_once =%d g_sd_power_dircect_ctrl = %d\n",
//					__func__, dev_id, enable, enabled_once[dev_id - 1], g_sd_power_dircect_ctrl);

  if (g_sd_power_dircect_ctrl && (dev_id == 4))
  {    
    /* skip check - enable & enable_once 
       dev_id == 4 -> SDCARD */
  }
  else
  {
	...
	printk(KERN_ERR "[sdcc_vreg] %s: dev_id = %d enable = %d enable_once =%d dircect_ctrl = %d\n",
					__func__, dev_id, enable, enabled_once[dev_id - 1], g_sd_power_dircect_ctrl);
...
#ifdef CONFIG_MACH_LGE_M3S // Write Protect Disable
	.wpswitch    = NULL,
#else /*CONFIG_MACH_LGE_M3S*/
...
#ifdef CONFIG_MACH_LGE_M3S  // LS696_me : SD card Power
...
static int check_smem_ers_status(void)
{
     ...
     * reserved
    */
    printk(KERN_ERR "[SMEM_CHECK] start \n");
    ...
    if(smem_sreset_hwreset != NULL)
    {
        ...
        printk(KERN_INFO "[SMEM_CHECK] : addr : 0x%X, value : 0x%X\n", (int)smem_sreset_hwreset, *smem_sreset_hwreset);
        ...
        if(*smem_sreset_hwreset == 0xDDDEADDD)
        {
            ...
            //phMscd_Filp = filp_open("/proc/last_kmsg_arm9", O_WRONLY | O_CREAT | O_TRUNC, 0);
            //if(IS_ERR(phMscd_Filp)) {
			//  printk(KERN_ERR "%s, open failed\n", "/proc/last_kmsg_arm9");
            //  goto handle_smem_sreset_err;
            //}
        ...
        else if(*smem_sreset_hwreset == 0xDDDDDEAD)
        {
            ...
            //phMscd_Filp = filp_open("/proc/first_bootmsg", O_WRONLY | O_CREAT | O_TRUNC, 0);
            //if(IS_ERR(phMscd_Filp)) {
            //  printk(KERN_ERR "%s, open failed\n", "/proc/first_bootmsg");
            //  goto handle_smem_sreset_err;
            //}
            ...
            printk(KERN_ERR "[SMEM_CHECK] CREATE first_bootmsg \n");
...
#endif /*CONFIG_LGE_DLOAD_RESET_BOOT_UP*/
    ...
    {
        printk(KERN_ERR "[SMEM_CHECK] ALLOC FAIL : 0x%X \n", (int)smem_sreset_hwreset);
...
/* LGE_S sangyup.kim@lge.com parsing kcal from cmdline */
int g_kcal[6];
...
//LGE_CHANGE_E
	...
	lge_m3s_audio_init();
#endif
//LGE_CHANGE_S CAMERA FIRMWARE UPDATE (tao.jin@lge.com)
//LGE_CHANGE_S CAMERA FIRMWARE UPDATE (jongkwon.chae@lge.com)
	lge_add_camera_devices();
//LGE_CHANGE_E CAMERA FIRMWARE UPDATE (jongkwon.chae@lge.com)
//LGE_CHANGE_E CAMERA FIRMWARE UPDATE (tao.jin@lge.com)
